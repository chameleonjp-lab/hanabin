...
> 2026年9月8日追記（品質改善Draft PR #48〜#53）: 上記計画のQ1〜Q6を依存順で実装した。入力座標・縦画面・HUD予約領域、練習状態・連鎖適格性、ルール別記録、終端予告境界、同一SHA公開ゲート、音声復帰・終了表示を含む。統合ブランチでは構文81ファイル、Node 179成功/1スキップ、10,000シード安全検査、8,000ランのプレイ曲線、成果物SHA検査が成功している。Draft PRはmainへ未反映で、iPhone 17 Pro、実機音声、初見5人の受入、GitHub Actionsの各PR検査は継続中である。MVP受入は6/7のままとする。
...
2. 初見5人の試遊を記録する。
3. ルール数値を変更する場合は、人間の実測後に別の`ruleVersion`更新として扱う。
