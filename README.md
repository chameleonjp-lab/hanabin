...
M1〜M7（決定的判定、Canvas版、ゲームプレイ硬化、独自花火表現、製品導線、公開候補の自動検証）は`main`へマージ済みです。現在はM7の公開確認中で、公開URLはホームから本編画面まで確認済みです。結果画面、実機・初見試遊・Pages設定確認を残しています。ランキングはまだ実装していません。
...
- M7実装: Pull Request [#11](https://github.com/chameleonjp-lab/hanabin/pull/11)を`main`へマージ済み。公開版固定情報、最終自動検証、GitHub Pages準備を確認済み。公開確認記録はPull Request [#13](https://github.com/chameleonjp-lab/hanabin/pull/13)で`main`へ反映済み
